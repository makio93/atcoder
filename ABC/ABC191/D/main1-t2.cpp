...
int main(){
    ...
    ll ans = 0, tpos = (ll)(floor((double)x/(ll)(1e4))) * (ll)(1e4);
    for (ll i=tpos; i>=x-r; i-=(ll)(1e4)) {
        ll dx = llabs(i-x), dy = sqrtl((__int128)r*r-(__int128)dx*dx), ytop = (ll)(floor((double)(y+dy)/(ll)(1e4)))*(ll)(1e4), ybot = (ll)(floor((double)(y-dy-1)/(ll)(1e4)))*(ll)(1e4);
        ans += max(0LL, (ytop-ybot)/(ll)(1e4));
    }
