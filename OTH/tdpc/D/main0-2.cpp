...
int main(){
    ...
    rep(i, n) {
        ...
        rep(ai, 80) rep(bi, 80) rep(ci, 80) {
            if (dp[ai][bi][ci] == 0) continue;
            tmp[ai+1][bi][ci] += dp[ai][bi][ci];
            tmp[ai+2][bi][ci] += dp[ai][bi][ci];
            tmp[ai][bi+1][ci] += dp[ai][bi][ci];
            tmp[ai][bi][ci+1] += dp[ai][bi][ci];
            tmp[ai+1][bi+1][ci] += dp[ai][bi][ci];
        }
        swap(dp, tmp);
    }
    double ans = 0.0;
